...
`nfsmon.conf.example`. The relevant sections:
...
1. **`ss -tniH state established '( sport = :<port> )'`**
   - Per established connection: peer (client) IP/port, bytes sent
     (`bytes_acked`, falling back to `bytes_sent`), `bytes_received`,
     smoothed `rtt` from the `-i` flag (in ms)
   - Aggregation per client IP: connections summed, bytes summed,
     RTT averaged
...
- **NFSv3 clients are not shown.** `nfsmon` filters by default on
  TCP port 2049 (the standard NFSv4 port); v3 runs over
